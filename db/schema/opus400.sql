...
CREATE  TABLE IF NOT EXISTS `opus400`.`documents` (
  ...
  `completed_year` YEAR NOT NULL COMMENT 'Year of completion, if the \"completeted_date\" (exact date) is unknown. / Jahr der Fertigstellung, wenn das \"completeted_date\" (vollständiges Datum) unbekannt ist.' ,
  ...
  `publication_state` ENUM('published', 'unpublished','deleted') NOT NULL COMMENT 'Status of publication prozess in the repository. / Status im Publikationsprozess im Repository.' ,
  `published_date` DATE NULL COMMENT 'Exact date of publication. Could differ from \"server_date_published\". / Exaktes Datum der Publikation. Kann sich vom \"server_date_published\" unterscheiden.' ,
  `published_year` YEAR NULL COMMENT 'Year of the publication, if the \"published_date\" (exact date) is unknown.  Could differ from \"server_date_published\". / Jahr der Publikation, wenn das \"published_date\" (vollständiges Datum) unbekannt ist. Kann sich vom \"server_date_published\" unterscheiden.' ,
...
CREATE  TABLE IF NOT EXISTS `opus400`.`document_subjects` (
  `document_subjects_id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Primary key. / Primärschlüssel.' ,
  `documents_id` INT UNSIGNED NULL COMMENT 'Foreign key to: documents.documents_id / Fremdschlüssel zu: documents.documents_id ' ,
  `subject_language` VARCHAR(3) NULL COMMENT 'Language of the subject heading (triple-digit, ISO639-2/B). / Sprache des Schlagworts (dreistellig, ISO639-2/B).' ,
  `subject_type` ENUM('swd', 'psyndex', 'uncontrolled') NOT NULL COMMENT 'Subject type, i. e. a specific authority file. / Art des Schlagwortes, z. B. eine spezifische Normdatei.' ,
  `subject_value` VARCHAR(255) NOT NULL COMMENT 'Value of the subject heading, i. e. text, notation etc. / Wert des Schlagwortes, z. B. Text, Notation etc.' ,
  `external_subject_key` VARCHAR(255) NULL COMMENT 'Identifier for linking the subject heading to external systems such as authority files. / Identifikator zur Verknüpfung des Schlagworts mit externen Systemen wie z. B. Normdateien.' ,
...
CREATE  TABLE IF NOT EXISTS `opus400`.`document_patents` (
  `document_patents_id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Primary key. / Primärschlüssel.' ,
  `documents_id` INT UNSIGNED NOT NULL COMMENT 'Foreign key to: documents.documents_id / Fremdschlüssel zu: documents.documents_id ' ,
  `patent_countries` TEXT NOT NULL COMMENT 'Countries in which the patent was granted. / Länder, in denen das Patent erteilt wurde.' ,
  `patent_date_granted` DATE NOT NULL COMMENT 'Date when the patent was granted. / Datum der Patenterteilung.' ,
  `patent_number` VARCHAR(255) NOT NULL COMMENT 'Patent number. / Patent- bzw. Publikationsnummer.' ,
  `patent_year_applied` YEAR NOT NULL COMMENT 'Year of the application. / Jahr der Antragsstellung.' ,
  `patent_application` TEXT NOT NULL COMMENT 'Description of the patent. / Beschreibung des Patents.' ,
...
CREATE  TABLE IF NOT EXISTS `opus400`.`document_notes` (
  `document_notes_id` INT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Primary key. / Primärschlüssel.' ,
  `documents_id` INT UNSIGNED NOT NULL COMMENT 'Foreign key to: documents.documents_id / Fremdschlüssel zu: documents.documents_id ' ,
  `message` TEXT NOT NULL COMMENT 'Message text. / Mitteilungstext.' ,
  `creator` TEXT NOT NULL COMMENT 'Crator of the message. / Verfasser der Mitteilung.' ,
  `scope` ENUM('private', 'public', 'reference') NOT NULL COMMENT 'Visibility: private, public, reference to another document version. / Sichtbarkeit: intern, extern, Verweis auf andere Dokumentversion.' ,
...
CREATE  TABLE IF NOT EXISTS `opus400`.`document_licences` (
  ...
  `link_licence` MEDIUMTEXT NOT NULL COMMENT 'URI of the licence text. / URI des Lizenztextes.' ,
  ...
  `mime_type` VARCHAR(30) NOT NULL COMMENT 'Mime type of the licence text linked in \"link_licence\". / Mime-type des Lizenztextes der in \"link_licence\" angegeben ist.' ,
  `name_long` VARCHAR(255) NOT NULL COMMENT 'Full name of the licence as displayed to users. / Kompletter Name der Lizenz. Dieser wird Nutzern angezeigt.' ,
...
CREATE  TABLE IF NOT EXISTS `opus400`.`link_documents_licences` (
  `documents_id` INT UNSIGNED NOT NULL COMMENT 'Primary key and foreign key to: documents.documents_id / Primärschlüssel und Fremdschlüssel zu: documents.documents_id ' ,
  `licences_id` INT UNSIGNED NOT NULL COMMENT 'Primary key and foreign key to: licences.licences_id / Primärschlüssel und Fremdschlüssel zu:  licences.licences_id ' ,
...
CREATE  TABLE IF NOT EXISTS `opus400`.`link_institutes_documents` (
  `institutes_id` INT UNSIGNED NOT NULL ,
  `documents_id` INT UNSIGNED NOT NULL ,
  `role` ENUM('publisher','creator','other') NOT NULL ,
