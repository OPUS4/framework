...
export MYSQL_PWD=$root_pwd && mysql --default-character-set=utf8 -h 'localhost' -P '3306' -u 'root' -v -e "CREATE DATABASE IF NOT EXISTS $database_name DEFAULT CHARACTER SET = UTF8 DEFAULT COLLATE = UTF8_GENERAL_CI; CREATE USER IF NOT EXISTS '$opus_admin'@'localhost' IDENTIFIED WITH mysql_native_password BY '$opus_admin_pwd'; GRANT ALL PRIVILEGES ON $database_name.* TO '$opus_admin'@'localhost'; CREATE USER IF NOT EXISTS '$opus_user'@'localhost' IDENTIFIED WITH mysql_native_password BY '$opus_user_pwd'; GRANT SELECT,INSERT,UPDATE,DELETE ON $database_name.* TO '$opus_user'@'localhost'; FLUSH PRIVILEGES;"
